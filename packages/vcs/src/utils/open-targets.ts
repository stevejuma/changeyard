...
export const PREFERRED_OPEN_TARGET_STORAGE_KEY = "kanban.preferred-open-target";
...
const OPEN_TARGET_OPTIONS: readonly OpenTargetOption[] = [
	DEFAULT_OPEN_TARGET,
	{ id: "vscode-insiders", label: "VS Code Insiders", iconSrc: vscodeIcon },
	{ id: "cursor", label: "Cursor", iconSrc: cursorIcon },
	{ id: "windsurf", label: "Windsurf", iconSrc: windsurfIcon },
	{ id: "finder", label: "Finder", iconSrc: finderIcon },
	{ id: "terminal", label: "Terminal", iconSrc: terminalIcon },
	{ id: "iterm2", label: "Iterm2", iconSrc: iterm2Icon },
	{ id: "ghostty", label: "Ghostty", iconSrc: ghosttyIcon },
	{ id: "warp", label: "Warp", iconSrc: warpIcon },
	{ id: "xcode", label: "Xcode", iconSrc: xcodeIcon },
	{ id: "intellijidea", label: "Intellij Idea", iconSrc: intellijIdeaIcon },
	{ id: "zed", label: "Zed", iconSrc: zedIcon },
];

const OPEN_TARGET_IDS_BY_PLATFORM: Record<OpenTargetPlatform, readonly OpenTargetId[]> = {
	mac: ["vscode", "cursor", "windsurf", "finder", "terminal", "iterm2", "ghostty", "warp", "xcode", "intellijidea", "vscode-insiders", "zed"],
...
function getDefaultOpenTargetId(platform: OpenTargetPlatform): OpenTargetId {
	return OPEN_TARGET_IDS_BY_PLATFORM[platform][0] ?? DEFAULT_OPEN_TARGET.id;
}
...
function getOpenTargetLabel(targetId: OpenTargetId, platform: OpenTargetPlatform): string {
	if (targetId === "finder") {
		...
		if (platform === "linux" || platform === "other") return "File Manager";
	}
	return openTargetById.get(targetId)?.label ?? DEFAULT_OPEN_TARGET.label;
}
...
function isOpenTargetId(value: string | null): value is OpenTargetId {
	return Boolean(value && openTargetById.has(value as OpenTargetId));
}

export function normalizeOpenTargetId(value: string | null): OpenTargetId | null {
	if (!value) return null;
	if (value === "ghostie") return "ghostty";
	if (value === "intellij_idea") return "intellijidea";
	return isOpenTargetId(value) ? value : null;
}
...
function buildOpenAppCommand(path: string, ...appNames: string[]): string {
	...
	return openAttempts.length === 1 ? (openAttempts[0] ?? `open ${quotedPath}`) : `(${openAttempts.join(" || ")})`;
}

function buildOpenLinuxCommand(targetId: OpenTargetId, path: string): string {
	const quotedPath = quoteShellArgument(path);
	if (targetId === "finder") return `xdg-open ${quotedPath}`;
	if (targetId === "vscode") return `code ${quotedPath}`;
	if (targetId === "vscode-insiders") return `code-insiders ${quotedPath}`;
	if (targetId === "cursor") return `cursor ${quotedPath}`;
	if (targetId === "windsurf") return `windsurf ${quotedPath}`;
	if (targetId === "zed") return `zed ${quotedPath}`;
...
function buildOpenWindowsCommand(targetId: OpenTargetId, path: string): string {
	...
	if (targetId === "finder") return `explorer ${quotedPath}`;
	if (targetId === "vscode") return `code ${quotedPath}`;
	if (targetId === "vscode-insiders") return `code-insiders ${quotedPath}`;
	if (targetId === "cursor") return `cursor ${quotedPath}`;
	if (targetId === "windsurf") return `windsurf ${quotedPath}`;
...
export function getOpenTargetOptions(platform: OpenTargetPlatform): readonly OpenTargetOption[] {
	return OPEN_TARGET_IDS_BY_PLATFORM[platform].map((targetId) => {
		...
		return { ...option, label: getOpenTargetLabel(targetId, platform) };
	});
}
...
export function getOpenTargetOption(targetId: OpenTargetId, platform: OpenTargetPlatform): OpenTargetOption {
	...
	return { ...option, label: getOpenTargetLabel(resolvedTargetId, platform) };
}
...
export function loadPersistedOpenTarget(platform: OpenTargetPlatform): OpenTargetId {
	...
	if (typeof window === "undefined") return defaultTargetId;
	const normalized = normalizeOpenTargetId(window.localStorage.getItem(PREFERRED_OPEN_TARGET_STORAGE_KEY));
	return normalized && isOpenTargetSupported(normalized, platform) ? normalized : defaultTargetId;
}
...
export function buildOpenCommand(targetId: OpenTargetId, path: string, platform: OpenTargetPlatform): string {
	...
	if (platform === "windows") return buildOpenWindowsCommand(targetId, path);
	if (platform === "linux" || platform === "other") return buildOpenLinuxCommand(targetId, path);
	if (targetId === "vscode") return buildOpenAppCommand(path, "Visual Studio Code");
	if (targetId === "vscode-insiders") return buildOpenAppCommand(path, "Visual Studio Code - Insiders");
	if (targetId === "cursor") return buildOpenAppCommand(path, "Cursor");
	if (targetId === "windsurf") return buildOpenAppCommand(path, "Windsurf");
	if (targetId === "finder") return buildOpenAppCommand(path);
	if (targetId === "terminal") return buildOpenAppCommand(path, "Terminal");
	if (targetId === "iterm2") return buildOpenAppCommand(path, "iTerm", "iTerm2");
	if (targetId === "ghostty") return buildOpenAppCommand(path, "Ghostty", "Ghostie");
	if (targetId === "warp") return buildOpenAppCommand(path, "Warp");
	if (targetId === "xcode") return buildOpenAppCommand(path, "Xcode");
	if (targetId === "intellijidea") return buildOpenAppCommand(path, "IntelliJ IDEA", "IntelliJ IDEA CE");
