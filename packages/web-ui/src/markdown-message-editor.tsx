...
function readMarkdownColorMode(): "dark" | "light" {
...
function useMarkdownColorMode(): "dark" | "light" {
	const [colorMode, setColorMode] = useState(readMarkdownColorMode);

	useEffect(() => {
		if (typeof document === "undefined") {
			return;
		}
		const observer = new MutationObserver(() => setColorMode(readMarkdownColorMode()));
