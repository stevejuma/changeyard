...
import { DEFAULT_KANBAN_RUNTIME_HOST, DEFAULT_KANBAN_RUNTIME_PORT } from "./runtime-endpoint.js";
...
const CHANGEYARD_SHELL_METRICS_ARG = "--changeyard-shell-metrics=";
const ELECTRON_MACOS_SHELL_METRICS = {
	workspaceSafeAreaLeft: 62,
};

function resolveBuiltArtifact(...candidates: string[]): string {
...
function getFrontendOrigin(): string | null {
	const configured = process.env.CHANGEYARD_DESKTOP_WEB_UI_URL?.trim();
	...
	try {
		...
		console.error(
			"[desktop] Invalid CHANGEYARD_DESKTOP_WEB_UI_URL; using runtime origin instead:",
...
function getShellMetrics(): { workspaceSafeAreaLeft?: number } {
...
function getShellArguments(): string[] {
	...
	return [
		`${CHANGEYARD_SHELL_METRICS_ARG}${encodeURIComponent(JSON.stringify(shellMetrics))}`,
	];
}

const preloadPath = resolveBuiltArtifact(
	path.join(import.meta.dirname, "preload.js"),
	path.join(import.meta.dirname, "..", "preload.js"),
	path.join(process.cwd(), "dist", "preload.js"),
);
const disconnectedHtmlPath = resolveBuiltArtifact(
	path.join(import.meta.dirname, "disconnected.html"),
	path.join(import.meta.dirname, "..", "disconnected.html"),
	path.join(process.cwd(), "dist", "disconnected.html"),
);
...
const windowFactory = new WindowFactory({
	...
	additionalArguments: getShellArguments(),
...
orchestrator.on("url-changed", (url) => {
	if (url) {
		const uiUrl = orchestrator.getFrontendUrl() ?? url;
		registry.loadUrlInAllWindows(uiUrl).catch((err) => {
			console.error(
...
function resolveCliShimPath(): string {
	...
	const candidates = [
		path.join(import.meta.dirname, "..", "build", "bin", devShimName),
		path.join(import.meta.dirname, "..", "..", "build", "bin", devShimName),
		path.join(process.cwd(), "build", "bin", devShimName),
	];
	for (const candidate of candidates) {
