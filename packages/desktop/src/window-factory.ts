...
export class WindowFactory {
	...
	create(options: CreateWindowOptions = {}): BrowserWindow {
		...
		const frontendUrl = this.opts.orchestrator.getFrontendUrl() ?? runtimeUrl;
		if (frontendUrl) {
			const url = buildWindowUrl(frontendUrl, options);
			...
			const lastUrl = window.webContents.getURL();
				...
				void this.opts.orchestrator.checkHealth(origin).then((healthy) => {
					...
					if (healthy && runtimeUrl) {
						const frontendUrl =
							this.opts.orchestrator.getFrontendUrl() ?? runtimeUrl;
						const target = pickRecoveryUrl(lastUrl, frontendUrl);
