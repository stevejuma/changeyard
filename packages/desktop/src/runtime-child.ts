/**
 * RuntimeChildManager — spawns the Changeyard CLI as a subprocess, polls for
...
export class RuntimeChildManager extends EventEmitter<RuntimeChildManagerEvents> {
	private readonly defaultHost = DEFAULT_KANBAN_RUNTIME_HOST;
	private readonly defaultPort = DEFAULT_KANBAN_RUNTIME_PORT;
	...
	private async spawnChild(config: RuntimeChildConfig): Promise<string> {
		...
		const host = config.host?.trim() || this.defaultHost;
		...
		const args = ["hub", "run", "--no-open", "--port", String(port), "--host", host];
