 ...
 */
const KANBAN_RUNTIME_TITLE_MARKERS = ["<title>Kanban</title>", "<title>Changeyard</title>"];
...
export class RuntimeOrchestrator extends EventEmitter<RuntimeOrchestratorEventMap> {
	...
	private readonly frontendOrigin: string | null;
	...
	constructor(private readonly opts: RuntimeOrchestratorOptions) {
		super();
		this.host = opts.host ?? DEFAULT_KANBAN_RUNTIME_HOST;
		this.port = opts.port ?? DEFAULT_KANBAN_RUNTIME_PORT;
		this.frontendOrigin = opts.frontendOrigin?.trim() || null;
	}
	...
	getFrontendUrl(): string | null {
		...
		return this.frontendOrigin ?? this.url;
	}
	...
	async checkHealth(origin: string): Promise<boolean> {
		...
		try {
			...
			// See `KANBAN_RUNTIME_TITLE_MARKERS` for why a body match is
			// required.
			const body = await res.text();
			return KANBAN_RUNTIME_TITLE_MARKERS.some((title) => body.includes(title));
		} catch {
	...
	private async startOwnRuntime(): Promise<void> {
		...
		if (this.terminated) return;
		const runtimeOrigin = this.defaultOrigin();
		if (!this.manager) {
		...
		try {
			...
			if (this.isAddressInUseError(err) && (await this.checkHealth(runtimeOrigin))) {
				console.log(
					`[desktop] Runtime port already in use at ${runtimeOrigin}; attaching to existing runtime.`,
				);
				this.lastKnownOrigin = runtimeOrigin;
				this.setUrl(runtimeOrigin, /* owns */ false);
				return;
			}
		...
		if (!existsSync(resolved)) {
			throw new Error(
				`CLI shim not found at ${resolved}. ` +
					`In dev, run \`pnpm --filter @changeyard/desktop run stage:cli\` to populate build/bin/. ` +
