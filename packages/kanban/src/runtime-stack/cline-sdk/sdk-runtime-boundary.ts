...
export function loadClineSdkRulesForSystemPrompt(service: ClineSdkUserInstructionService): string {
	const ruleRecords = service.listRecords<RuleConfig>("rule") as Array<{ item: RuleConfig }>;
	const rules = ruleRecords
		.map((record: { item: RuleConfig }) => record.item)
		.filter(isRuleEnabled)
		.sort((left: RuleConfig, right: RuleConfig) => left.name.localeCompare(right.name));
