...
import type { AgentTool } from "@clinebot/agents";
import {
	addLocalProvider,
...
async function listRefreshedCatalogProviderModels(
	...
	const catalogProviderIds = ClineCore.Llms.resolveProviderModelCatalogKeys(providerId) as string[];
	const resolvedCatalogs = await Promise.all(
		catalogProviderIds.map((catalogProviderId: string) =>
			resolveProviderConfig(
				catalogProviderId,
	...
	return resolvedCatalogs.flatMap((resolvedCatalog: SdkProviderConfig | undefined) =>
		Object.entries(resolvedCatalog?.knownModels ?? {}).map(([modelId, model]: [string, SdkResolvedProviderModel]) =>
...
export async function listSdkProviderModels(providerId: string): Promise<SdkProviderModel[]> {
	...
	const [localModels, refreshedModels] = await Promise.all([
		getLocalProviderModels(providerId, config)
			.then((response: Awaited<ReturnType<typeof getLocalProviderModels>>) => response.models.map(toSdkProviderModel))
			.catch(() => []),
