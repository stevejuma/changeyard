...
export const TASK_START_ONBOARDING_SLIDES: OnboardingSlide[] = [
	{
		...
		assetImageUrl: "/assets/onboarding/changeyard-create-tasks.png",
	...
	},
	{
		...
		assetImageUrl: "/assets/onboarding/changeyard-auto-commit-link.png",
		assetAlt: "Linking task cards in ChangeYard Kanban",
		assetWidthPx: 1600,
		assetHeightPx: 900,
	},
	{
		...
		assetImageUrl: "/assets/onboarding/changeyard-review-comments.png",
		assetAlt: "Leaving comments on code diffs in ChangeYard Kanban",
		assetWidthPx: 1600,
		assetHeightPx: 900,
	},
	{
...
const ONBOARDING_AGENT_IDS: readonly RuntimeAgentId[] = ["cline", "cursor", "claude", "codex", "copilot", "droid", "kiro"];
...
function resolveInstallInstructions(agentId: RuntimeAgentId): string {
	...
	if (agentId === "copilot") {
		return "GitHub's coding agent CLI with Copilot models and GitHub integration.";
	}
	if (agentId === "cursor") {
		return "Cursor's terminal agent CLI with access to frontier models and MCP integrations.";
	}
	if (agentId === "droid") {
...
export function TaskStartAgentOnboardingCarousel({
	...
	const handleDoneAction = useCallback(async (): Promise<OnboardingDoneResult> => {
		...
		if (!saveResult.ok) {
			const message = saveResult.message ?? "Could not save ChangeYard provider settings.";
