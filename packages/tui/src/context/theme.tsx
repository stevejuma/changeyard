...
import carbonfox from "./theme/carbonfox.json";
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode?: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      mode: kv.get("theme_mode", props.mode ?? "dark") as "dark" | "light",
...
function loadCustomThemes(): Record<string, ThemeJson> {
  ...
  const directories = [path.join(homedir(), ".changeyard")];
  let dir = process.cwd();
  while (true) {
    const candidate = path.join(dir, ".changeyard");
    if (existsSync(candidate) && !directories.includes(candidate)) {
  ...
  for (const base of directories) {
    ...
    for (const file of readdirSync(themesDir)) {
      ...
      try {
        result[name] = JSON.parse(readFileSync(path.join(themesDir, file), "utf8")) as ThemeJson;
      } catch {
