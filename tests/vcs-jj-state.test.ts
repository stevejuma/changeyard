...
test("loadJjState returns grouped stacks from one bounded graph read and working-copy summaries", async () => {
	...
	const result = await loadJjState("/repo", async ({ command, args }) => {
		...
		switch (joined) {
			...
			case 'jj bookmark list --revisions mine() ~ "main" --template name ++ "\\t" ++ self.normal_target().change_id().shortest(12) ++ "\\t" ++ self.normal_target().commit_id().shortest(12) ++ "\\t" ++ if(self.synced(), "1", "0") ++ "\\t" ++ if(self.tracked(), "1", "0") ++ "\\n"':
				return {
					ok: true,
			...
			case 'jj log --revisions connected("main"::("feature/api" | "feature/ui")) --no-graph --template change_id.shortest(12) ++ "\\t" ++ commit_id.shortest(12) ++ "\\t" ++ description.first_line().replace("\\\\t", " ").replace("\\\\n", " ") ++ "\\t" ++ parents.map(|p| p.change_id().shortest(12)).join("|") ++ "\\t" ++ local_bookmarks.map(|b| b.name()).join("|") ++ "\\t" ++ remote_bookmarks.map(|b| separate("@", b.name(), b.remote())).join("|") ++ "\\t" ++ if(current_working_copy, "1", "0") ++ "\\n"':
				return {
					ok: true,
					stdout: [
	...
	assert.equal(calls.filter((call) => call.startsWith("jj log --revisions connected(")).length, 1);
});
