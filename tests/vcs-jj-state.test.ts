...
test("loadJjState uses configured remote target as the base boundary", async () => {
	...
	const result = await loadJjState(
		...
		async ({ command, args }) => {
			...
			switch (joined) {
				...
				case "gh auth status --hostname github.com":
					return { ok: true, stdout: "Logged in", stderr: "", exitCode: 0 };
				case 'jj bookmark list --ignore-working-copy --at-op=@ --revisions all() ~ ::trunk@origin --template name ++ "\\t" ++ self.normal_target().change_id().shortest(12) ++ "\\t" ++ self.normal_target().commit_id().shortest(12) ++ "\\t" ++ if(self.synced(), "1", "0") ++ "\\t" ++ if(self.tracked(), "1", "0") ++ "\\n"':
					return {
						ok: true,
				...
				case 'jj log --ignore-working-copy --at-op=@ --revisions (::"feature/top") ~ ::trunk@origin --no-graph --template change_id.shortest(12) ++ "\\t" ++ commit_id.shortest(12) ++ "\\t" ++ description.first_line().replace("\\\\t", " ").replace("\\\\n", " ") ++ "\\t" ++ author.name().replace("\\\\t", " ").replace("\\\\n", " ") ++ "\\t" ++ author.email() ++ "\\t" ++ parents.map(|p| p.change_id().shortest(12)).join("|") ++ "\\t" ++ local_bookmarks.map(|b| b.name()).join("|") ++ "\\t" ++ remote_bookmarks.map(|b| separate("@", b.name(), b.remote())).join("|") ++ "\\t" ++ if(current_working_copy, "1", "0") ++ "\\n"':
					return {
						ok: true,
	...
	assert.ok(calls.includes('jj bookmark list --ignore-working-copy --at-op=@ --revisions all() ~ ::trunk@origin --template name ++ "\\t" ++ self.normal_target().change_id().shortest(12) ++ "\\t" ++ self.normal_target().commit_id().shortest(12) ++ "\\t" ++ if(self.synced(), "1", "0") ++ "\\t" ++ if(self.tracked(), "1", "0") ++ "\\n"'));
});
