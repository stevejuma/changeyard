...
const DEV_WEB_PORT = Number(process.env.KANBAN_WEB_UI_PORT || "4173");
const DEV_HOST = process.env.KANBAN_WEB_UI_HOST || "127.0.0.1";
...
async function main(): Promise<void> {
	...
	const viteChild = spawn(
		...
		["--dir", "packages/kanban/web-ui", "run", "dev", "--", "--host", DEV_HOST, "--port", String(DEV_WEB_PORT)],
		{
			...
			env: {
				...
				KANBAN_RUNTIME_PORT: runtimeUrl.port,
				KANBAN_WEB_UI_PORT: String(DEV_WEB_PORT),
				KANBAN_WEB_UI_HOST: DEV_HOST,
			},
		},
	);
	...
	const shutdown = async (signal?: NodeJS.Signals): Promise<void> => {
		...
		shuttingDown = true;
		if (signal) {
			process.stderr.write(`\nShutting down ui:dev after ${signal}...\n`);
		}
	...
	process.stdout.write(`\nChangeyard source UI dev server\n`);
	process.stdout.write(`Runtime API: ${runtimeUrl.origin}\n`);
	process.stdout.write(`Web UI (HMR): ${devUrl}\n\n`);
}
